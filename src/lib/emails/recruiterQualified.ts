export function recruiterQualifiedEmail(params: {
  recruiterName: string;
  artistName: string;
  amount: number;
}) {
  const { recruiterName, artistName, amount } = params;
      ...
      <div style="background-color: #1A1A1A; padding: 40px 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
        <div style="max-width: 460px; margin: 0 auto;">
          ...
          <div style="background-color: #242424; border-radius: 12px; padding: 32px; border: 1px solid #333;">
            <h2 style="color: #FFFFFF; font-size: 20px; font-weight: 600; margin: 0 0 12px 0;">You got paid, ${recruiterName}!</h2>
            <div style="text-align: center; margin: 24px 0;">
              <p style="color: #D4AF37; font-size: 36px; font-weight: bold; margin: 0;">$${(amount / 100).toFixed(2)}</p>
              <p style="color: #666; font-size: 12px; margin: 4px 0 0 0;">sent to your Stripe account</p>
            </div>
            <p style="color: #A0A0A0; font-size: 14px; line-height: 1.6; margin: 0 0 24px 0;">
              <strong style="color: #FFFFFF;">${artistName}</strong> completed their 30-day qualifying period on a paid plan. Your flat fee has been sent to your connected Stripe account.
            </p>
            <p style="color: #A0A0A0; font-size: 14px; line-height: 1.6; margin: 0 0 24px 0;">
              Keep referring artists to unlock higher payouts and recurring commissions.
            </p>
