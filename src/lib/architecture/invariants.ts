...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase3-distribution-page-index.sql', state: 'pending', note: 'Big Page Index upgrade: four additive metadata columns on distribution_pages (first_discovered_at, discovery_source, index_eligible, last_posts_refresh_at) plus the distribution_page_posts recent-post corpus (public captions stored deliberately so future artist searches can match against the cached corpus; admin-only, same lockdown). Every code path survives it being pending: searches run global-only, upsertPages falls back to the legacy shape on missing columns, and the index panel reports migration_pending instead of erroring.' },
