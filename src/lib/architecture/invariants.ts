...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase2-free-join-sequence-trigger.sql', state: 'pending', liveCheck: 'sql-check', note: "Widens sequences_trigger_type_check to admit free_join (plus abandoned_cart and loyalty_survey, which webhookHandlers has always emitted and the constraint never listed). Until applied, an artist cannot SAVE a sequence on the free-join trigger, so the enrolment call in freeJoin.ts simply finds no active sequence and returns; a free join still works and nothing throws. Opt-in either way: no artist has a free_join sequence until they build one." },
  { file: 'schema-phase2-member-files.sql', state: 'pending', note: "One table, member_files (files jsonb of PRIVATE object keys, allowed_tier_ids jsonb). RLS enabled with NO client grants: anon and authenticated are revoked by name, so every read and write goes through the service-role routes that resolve the artist from the session. Deliberately holds KEYS, never URLs; the self-verify asserts RLS on, zero client grants, and no column whose name contains 'url'. Until applied, /api/member-files answers { bundles: [], pending: true } and the Studio panel says member downloads are not switched on; no other surface is affected." },
  { file: 'schema-phase2-product-file-privacy.sql', state: 'pending', note: "Adds products.file_key (PRIVATE object key). Does NOT revoke products.file_url: src/app/[slug]/page.tsx reads products with select('*'), and a revoked column 42501s the whole statement, which would empty every storefront. The leak is closed at the SOURCE instead, in the same commit: neither upload site creates a public URL any more, and delivery goes through /api/products/download after a purchase check. Probe 2026-08-31 found 8 products and ZERO carrying a file_url, so no historical row needs rewriting. Until applied, a digital product's file save fails on the missing column and the artist is told the upload failed rather than shipping a product that claims a file it cannot deliver." },
];
