...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase2-sequence-conversion-goal.sql', state: 'pending', note: "sequences.goal_tier_id: the optional PAID tier a sequence is selling. A fan holding an active subscription at or above it (price rank via tierLadder.ts, never a tier name) exits the sequence as converted: immediately in the Stripe webhook, self-healed by the daily sequences cron, and refused at enrollment. Same-artist and paid-only are enforced by a DB trigger AND at the API. Null = legacy sequence, behavior byte-for-byte unchanged; every reader retries without the column pre-migration, so nothing breaks while this waits." },
  { file: 'schema-phase2-fan-funnel-foundation.sql', state: 'pending', liveCheck: 'sql-check', note: "Build 1 of the reusable fan sales engine: attribution jsonb on fan_automation_leads and song_lab_offer_claims (first-touch, written only through campaignAttribution.ts normalization; descriptive only, never price/entitlement/redirect authority) and fan_automations.nurture_sequence_id (funnel-specific nurture pointer, same-artist trigger). sql-check because both fan-automation tables are revoked from anon, so a probe answers 42501 whether or not a column exists. Every writer retries without the new columns pre-migration: the lead, the claim and the automation all survive; only the tag or pointer is lost." },
