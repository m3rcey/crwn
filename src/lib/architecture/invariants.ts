...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase3-distribution-finder.sql', state: 'pending', note: 'Two admin-only tables (distribution_pages, distribution_mentions) behind the founder Artist Distribution Finder on /admin. Public Instagram observations only, service-role writes via requireAdmin routes, ALL revoked from anon. Every code path survives it being pending: a live Apify search still runs and returns results; only the 24h cache and the compounding artist-to-page graph are disabled until it is applied.' },
