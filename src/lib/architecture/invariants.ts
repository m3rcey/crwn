...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase3-social-publish-queue.sql', state: 'pending', note: 'Authored 2026-08-26 for scheduled Instagram publishing. One table, service-role only (RLS enabled with ZERO policies, ALL revoked from anon/authenticated), no money column and no credential column, both asserted by the self-verify block. Phase 0 already proved direct publishing works: media 18415895044156240 on @thecrwnapp. Until this runs, only the manual publisher (scripts/test-instagram-carousel-publish.mjs) can post.' },
