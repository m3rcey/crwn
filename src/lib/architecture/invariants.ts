...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase2-subscription-prize-campaign.sql', state: 'pending', note: "subscriptions.prize_campaign_id: a nullable FK marking a membership a CAMPAIGN PRIZE funded. Money truth was already correct (SEC-006 books gross from amount_total/amount_paid, so a fully discounted subscription writes $0), but paidMembers and mrrCents in constraint/assembler.ts derive from the TIER'S price, so a comped Platinum would report $50/month of recurring revenue nobody pays. ON DELETE SET NULL: deleting a campaign must never delete a membership. Additive, no backfill, every existing row NULL. The assembler retries its select without the column pre-migration, so nothing reports zero members while this waits." },
