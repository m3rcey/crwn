...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase2-featured-on-home-opt-in.sql', state: 'pending', liveCheck: 'anon-probe', note: 'artist_profiles.featured_on_home (default false) makes the Home Featured Artists row OPT-IN, founder decision 2026-09-26: a new signup is no longer shown to everyone at login until the founder features them. Home row only; Explore keeps reading featured_hidden, which was NOT defaulted on because it also removes an artist from Explore browse and tracks. Backfills gb and m3rcey by slug and rebuilds artist_profiles_public. Until applied, the tolerant Home opt-in query errors and the row falls back to the old completeness rule, so new artists still appear.' },
