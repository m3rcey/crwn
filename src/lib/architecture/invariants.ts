...
export const EXPECTED_MIGRATION_STATE: ReadonlyArray<{
  ...
  { file: 'schema-phase2-drop-new-artist-trigger.sql', state: 'applied', liveCheck: 'sql-check', note: 'Founder-applied 2026-09-26 (self-verify passed). Drops trg_notify_new_artist and notify_new_artist(). The founder new-artist alert moved in-process (src/lib/newArtistAlert.ts) on 2026-09-26 because the pg_net trigger plus a hand-pasted shared secret failed silently more than once, and the route it called is deleted, so until this runs the trigger only produces harmless 404s. Prints the last pg_net deliveries first so the real failure is recorded. Triggers are invisible to the anon key, hence sql-check.' },
  { file: 'schema-phase2-featured-on-home-opt-in.sql', state: 'applied', liveCheck: 'anon-probe', note: 'Founder-applied 2026-09-26, probe-verified: only gb and m3rcey read featured_on_home = true, fr35h false. artist_profiles.featured_on_home (default false) puts the Home Featured Artists row at founder discretion (decision 2026-09-26): a new signup starts off it, the founder switches artists on at /admin?tab=artists, and the freeze trigger stops artists setting it or featured_hidden themselves. Home row only; Explore keeps reading featured_hidden, which was NOT defaulted on because it also removes an artist from Explore browse and tracks. Backfills gb and m3rcey by slug and rebuilds artist_profiles_public. Until applied, the tolerant Home opt-in query errors and the row falls back to the old completeness rule, so new artists still appear.' },
  ...
  { file: 'schema-phase2-internal-sms-alert-consent.sql', state: 'pending', note: 'ONE small table, internal_sms_alert_consents, holding the consent of JNW Creative Enterprises, Inc. authorized personnel to receive internal speed-to-lead SMS alerts. Reuse was rejected for cause: sms_consent_log has artist_id UUID NOT NULL REFERENCES artist_profiles (this consent belongs to no artist) and an action CHECK listing four fan-marketing actions from the product removed 2026-07-31; acquisition_events is the LEAD outbox and is read by acquisition analytics. RLS on with NO policies plus grants revoked from anon and authenticated by name, because a consent log is a list of real personal phone numbers. Until applied, /api/sms-alert-consent still records every consent: the founder copy email is sent on every accepted submission and is durable evidence in its own right, and the route only reports failure when BOTH sinks fail, so the compliance page never claims a consent it did not keep.' },
  ...
  // Social publishing queue (scheduled Instagram carousels). PENDING until the founder runs it.
  // There is no fail-soft path here and there deliberately is not one: the queue IS the schedule,
  // so until the table exists, scripts/queue-carousels.mjs cannot write a row and every cron tick
  // finds nothing due. That is a dead feature rather than a degraded one, which is the honest
  ...
  { file: 'schema-phase3-social-publish-multiplatform.sql', state: 'applied', note: 'Founder-applied and live-probed 2026-08-26 (social_post_targets answers the anon key 42501, reads revoked as expected). The backfill carried the one published Instagram post across intact: target published, provider id 17997602090807765, permalink preserved, kind=carousel. Authored the same day. Adds social_posts.kind + payload, creates social_post_targets (one row per post per platform, own caption, own status), backfills one Instagram target per existing post, widens platform to instagram/facebook/x/tiktok/youtube/threads. Same lockdown as the queue: RLS on, zero policies, ALL revoked from anon/authenticated, no money or credential columns, all asserted by the self-verify block.' },
  ...
  { file: 'schema-phase3-campaign-winner-selection.sql', state: 'applied', note: "APPLIED 2026-09-04. fan_campaign_participants.selected_winner_at: WHO was recorded as a campaign's winner, plus a PARTIAL UNIQUE index on (campaign_id) WHERE selected_winner_at IS NOT NULL that makes one-winner-per-campaign a database fact rather than an application belief. CRWN RECORDS a winner determined outside the product; it has no drawing engine and builds none. A trigger freezes the column: anon and authenticated may never set or change it (anon UPDATE/DELETE on this table return 200 with an empty array, denied by matching zero rows rather than explicitly, so the quiet control was made loud), and service_role may record once but never change or clear, making append-only a database fact too. VERIFIED BEHAVIOURALLY IN PRODUCTION, twice and by two different callers: the migration's own self-verify (superuser with a faked request.jwt.claims) proved an authenticated client is blocked, service_role can record once, and clearing is refused; scripts/probe-winner-freeze.mjs then drove the real service-role PostgREST path and proved the second-winner refusal (23505 on idx_fan_campaign_participants_one_winner), that a recorded winner can be neither cleared nor changed (42501), and that a DIFFERENT campaign may still have its own winner, cleaning up to zero rows. Additive, nullable, no backfill; production carried ZERO participants." },
  { file: 'schema-phase2-subscription-prize-campaign.sql', state: 'applied', note: "APPLIED 2026-09-04: founder ran it (17 subscriptions, 0 prize rows), then the column read back live through the service role and verify:migrations reported zero pending. subscriptions.prize_campaign_id: a nullable FK marking a membership a CAMPAIGN PRIZE funded. Money truth was already correct (SEC-006 books gross from amount_total/amount_paid, so a fully discounted subscription writes $0), but paidMembers and mrrCents derived from the TIER'S price, so a comped Platinum would report $50/month nobody pays. ON DELETE SET NULL: deleting a campaign must never delete a membership. Additive, no backfill, every existing row NULL. Prize STATE (scheduled vs active) is derived from this column plus the existing pending_change_date in campaigns/prizeState.ts, the one rule all three MRR readers share; no second column was added." },
  ...
  { file: 'schema-phase2-tier-events-offer-vocabulary.sql', state: 'applied', liveCheck: 'sql-check', note: "APPLIED: a service-role read on 2026-09-03 found a tier_events row carrying the new vocabulary, which the old CHECK would have refused, so the constraint was widened. Widens tier_events event_type with tier_vsl_started and tier_offer_declined, the two high-signal offer events, reusing the existing fan-side spine instead of a parallel stack. tier_checkout_started remains server-recorded only; the beacon route allowlists which types a client may name. sql-check because a CHECK is invisible to PostgREST. Pre-migration the daily-unique insert fails the CHECK and the best-effort writer swallows it: analytics never breaks a page." },
  { file: 'schema-phase2-fan-automation-link-provider.sql', state: 'applied', liveCheck: 'sql-check', note: "APPLIED: a service-role read on 2026-09-03 found a fan_automations row with provider = 'link' (the first live link funnel), which the old CHECK would have refused. Widens fan_automations.provider to admit 'link', so a funnel whose traffic comes from a bio link, QR code or external tool can SAY what it is. What makes a funnel link-only at runtime is connection_id being null, which is unchanged and already true of the first such funnel; the comment matcher only routes events through automations that HAVE an active connection. sql-check because fan_automations is revoked from anon. The create route retries with the historical provider on 23514, so the wizard's link-only mode works before this is applied and only the label waits." },
  ...
  { file: 'schema-phase2-member-files.sql', state: 'applied', note: "Probe-verified 2026-09-01: anon SELECT on id, files AND allowed_tier_ids each answer 42501, and an anon INSERT answers 42501 too, where the same table answered 404 PGRST205 before the run. One table, member_files (files jsonb of PRIVATE object keys, allowed_tier_ids jsonb). RLS enabled with NO client grants: anon and authenticated are revoked by name, so every read and write goes through the service-role routes that resolve the artist from the session. Deliberately holds KEYS, never URLs; the self-verify asserts RLS on, zero client grants, and no column whose name contains 'url'. The fail-soft path in /api/member-files stays as a guard." },
  { file: 'schema-phase2-product-file-privacy.sql', state: 'applied', note: "Probe-verified 2026-09-01: products.file_key answers 200 to anon while a nonexistent column on the same table answers 42703, so the column is really there. Adds products.file_key (PRIVATE object key). Does NOT revoke products.file_url: src/app/[slug]/page.tsx reads products with select('*'), and a revoked column 42501s the whole statement, which would empty every storefront. The leak is closed at the SOURCE instead, in the same commit: neither upload site creates a public URL any more, and delivery goes through /api/products/download after a purchase check. Probe 2026-08-31 found 8 products and ZERO carrying a file_url, so no historical row needs rewriting." },
];
