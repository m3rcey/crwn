...
export const POPUPS: PopupDef[] = [
  // ---- Artist: get paid (highest stakes, blocks money flow) ----
  {
    ...
    body: 'Every sale hits a dead end until payouts are connected. About five minutes.',
  ...
  // Losing that contest defers it: `everyN` keeps it eligible later rather than cancelling it.
  {
    ...
    body: 'Most artists never get a first paid member on CRWN. Know one still renting their fanbase?',
  ...
  // ---- Artist: first broadcast (activation, protects retention) ----
  {
    ...
    body: 'Members who hear nothing forget to renew.',
  ...
  // below take over at the actual break-even lines.
  {
    ...
    body: `Past $${usd(PRO_BREAK_EVEN)} a month in sales, staying on Launch costs you more than Pro does.`,
  ...
  // Pro's subscription every month they stay.
  {
    ...
    body: `Your last 30 days crossed $${usd(PRO_BREAK_EVEN)}. Launch's ${TIER_LIMITS.starter.platformFeePercent}% now costs more each month than Pro.`,
  ...
  // ---- Artist: Scale break-even crossed (one-time, Pro artists only) ----
  {
    ...
    body: `Your last 30 days crossed $${usd(SCALE_BREAK_EVEN)}. Pro's ${TIER_LIMITS.pro.platformFeePercent}% now costs more each month than Scale.`,
  ...
  // accounts created after the change shipped (announcedAt), who met these limits as normal.
  {
    ...
    body: 'Launch now gives you 50 tracks and no member cap at all.',
  ...
  // that is the actual failure this fixes.
  {
    ...
    body: 'Your page looks unlocked because you own it, so a broken ladder stays invisible.',
  ...
  // only for music they can already stream is why memberships stall.
  {
    ...
    body: 'If your membership offers what Spotify already has, joining buys nothing.',
  ...
  // Skipped for accounts created on or after announcedAt: they met the one-move screen at signup.
  {
    ...
    body: 'Four next moves is the same as none. Rise Mode now names one.',
  ...
  // ---- Fan: back an artist (activation, starts money flow) ----
  {
    ...
    body: 'Streams and follows pay the artists you love almost nothing. Backing one directly does.',
  ...
  // reads it, including the venue or brand the artist is trying to convince.
  {
    ...
    body: 'An artist you support asked what this has been like. Nobody sees it unless you allow it.',
  ...
  // and no Goals button, so announcing it would send artists looking for nothing.
  {
    ...
    body: 'The fans who show up live had no way to pay you. Set a tip goal.',
  ...
  // is still dark. Announce-once: a feature launch is news exactly one time.
  {
    ...
    body: 'Four kinds of royalty are owed to you, and none pay out unless you are registered.',
  ...
  // looking for a control that isn't there. Loss-framed per the copy rule.
  {
    ...
    body: 'Fans want in the room while you work. Right now that earns you nothing.',
  ...
  // artist learned the old 16-tab strip and every one of them is affected.
  {
    ...
    body: 'The menu in the top left now lists every screen, payouts and billing included.',
  ...
  // because fans get stuck too. Loss-framed per the copy rule.
  {
    ...
    body: 'Every question you sit on is momentum lost. Support answers most of them instantly.',
  ...
  // outcompete a money pop-up, and once ever is exactly what "notice" means.
  {
    ...
    body: 'A live or producer session seat is final once the session happens, and refunded if the artist cancels.',
