...
export function MessageArtistButton({ artistId, artistSlug, isOwnProfile }: MessageArtistButtonProps) {
  ...
  const [state, setState] = useState<{ canMessage: boolean; reason: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/messages?artistId=${artistId}`)
      .then((r) => r.json())
      .then((data) => { if (!cancelled && !data.error) setState({ canMessage: data.canMessage, reason: data.reason }); })
      .catch(() => {});
