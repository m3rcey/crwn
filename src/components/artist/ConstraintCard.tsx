...
import type { DiagnosedConstraint } from '@/lib/constraint/types';
import { confidenceLabel, withReturnTo } from '@/lib/constraint/presentation';

/**
 * The diagnosis is now fetched ONCE by the page and handed down, rather than each card fetching
 * for itself. That is what lets the page give exactly one card the primary CTA: two components
 * independently deciding they are important is how three gold buttons ended up stacked.
 */
export function ConstraintCard({ constraint }: { constraint: DiagnosedConstraint | null }) {
  ...
  return (
    <div className="neu-raised rounded-2xl p-5 mb-6 border border-crwn-gold/40" data-tour="constraint">
      ...
      <div className="mt-4 border border-crwn-gold/40 bg-crwn-gold/5 rounded-xl p-4">
        ...
        {/* returnTo: acting on the one thing CRWN told you to do must bring you back here, so the
            engine can re-read and tell you what is next. Without it the artist lands in Studio and
            has to coordinate again, which is the same problem one screen later. */}
        <Link
          prefetch
