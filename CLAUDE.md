...
- **Route:** `src/app/setup/page.tsx` (+ `layout.tsx`). **ONE thing per screen** — the wizard is a flat list of screens (`SCREENS` in that file), in order: **Photo → Tagline → Tier → Track → Product**. Progress bar + four group chips (Profile/Monetize/Music/Shop) up top; `Continue` locked until the screen's data exists. Photo, Tagline, Track are **mandatory**; Tier, Product are **skippable** ("Skip for now"). Stripe Connect is create-a-tier-now / finish-later, never a hard block. Ends on a **share screen** (public `thecrwn.app/{slug}` link + copy + X/Facebook/WhatsApp) → "Enter CRWN" → dashboard + trimmed tour. Do NOT stack multiple asks on one screen — that was the original mistake; keep it one-field-per-screen.
