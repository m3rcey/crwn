...
Feature button; `/api/admin/artists`, session admin via `requireAdmin`). Both discovery flags are
frozen against artist writes in `freeze_artist_profiles_protected_cols`
([supabase/schema-phase2-featured-on-home-opt-in.sql](supabase/schema-phase2-featured-on-home-opt-in.sql)),
because that trigger is a denylist and a new column is artist-writable by default.
...
1. **Daily canary** — `/api/cron/onboarding-health` (cron `0 7 * * *`) creates a throwaway user, performs the REAL RLS `artist_profiles` insert, verifies the user was **promoted to `role: artist`**, exercises `validateUpload`, then deletes the user. **Emails joshn.wms@gmail.com the moment any step fails.** The canary's RLS insert sends no founder alert: that alert is sent in-process by the code that creates the row (`sendNewArtistAlerts` in [src/lib/newArtistAlert.ts](src/lib/newArtistAlert.ts)), not by a DB trigger. **Never move it back to a pg_net trigger**: the old trigger + hand-pasted shared secret broke silently more than once, and it fired before the chosen name was saved. If you change the publish or upload flow, keep this check in sync.
...
- **Every setup screen carries a way OUT of the account.** `SetupAccountBar`
  (`src/components/onboarding/SetupAccountBar.tsx`) prints "Signed in as <email>" plus one
  Sign out action, and it is mounted on all four full-screen returns in `src/app/setup/page.tsx`:
  the loading spinner, `PlanIntro`, the wizard header (sticky, so it is on screen for all twelve
  field screens) and `LaunchReview`. The wizard is a HARD GATE and renders no nav, so before this
  the only exit was "Not an artist? Continue as a supporter", which sits on screen ONE only:
  anyone past it who had signed up on the wrong account, or who met a shared device already signed
  in, had to clear cookies to leave. **One action, not two.** Signing out lands on `/login`, which
  is where another account is entered and where a new one is created, so "switch account" and
  "sign out" are the same movement; the identity line is what makes a wrong account discoverable,
  and the button's aria-label carries both jobs. Do not add a second button that calls the same
  handler, and do not drop this bar from a new full-screen setup phase.
