...
- **Suppression/consent:** `email_suppressions` and `fan_communication_prefs` implement email unsubscribe/suppression. `sms_consent_log` (and the other `sms_*` tables) are DORMANT since the SMS removal 2026-07-31: kept only as historical consent records, nothing reads or writes them. **The 2026-08-24 internal alert authorization did NOT reopen them.** The artist's consent to be contacted rides `acquisition_events` (versioned `CALL_CONSENT_TEXT` + timestamp, written by the call-request route). The internal recipient's consent to receive operational alerts was originally planned as a BUSINESS RECORD outside the product (A2P accepts paper/verbal consent for non-web opt-in). **Twilio rejected that on review, 2026-08-25**: it asked to SEE the opt-in experience, not a description of a form it could not inspect. So the consent now lives in-product at the public page `/sms-alert-consent`, backed by `POST /api/sms-alert-consent` and the `internal_sms_alert_consents` table (migration `supabase/schema-phase2-internal-sms-alert-consent.sql`). That table is RLS-on with NO policies and grants revoked from `anon`/`authenticated` by name, because a consent log is a list of real personal phone numbers; the probe treats `42501` as the PASS and a `200` as an incident. The disclosure text and its version are SERVER constants (`src/lib/sms/alertConsent.ts`) and a client-sent consent string is never stored, or the record would prove nothing. Still deliberately NOT built: any consent-management UI, any admin surface, and any outbound sender. `Confirmed`.
