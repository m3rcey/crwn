...
| **Z2** | ~~Homepage + calculator express the value distribution~~ **PARTIALLY DONE 2026-08-10.** Shipped: the homepage hero (the Opportunity Calculator's registry hero, which is the live H1), the four homepage marketing sections that carried stale framing, the `worth` hero (anti-streaming framing removed), the vault / share-to-earn / live-experience heroes, and root metadata ("all-in-one platform" removed). **Deferred:** result-component concentration copy, and the remaining 11 registry calculators, which keep their hooks and are unblocked | Z1 | Low. Copy only; no formula, event, experiment or continuation id touched |
