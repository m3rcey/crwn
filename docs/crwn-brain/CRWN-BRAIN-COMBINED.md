...
Experimental/dark-launched: admin autonomous agent · **Scale** plan (billable once its Stripe prices + env vars exist) · "coming soon" benefit items. **LIVE as of the 2026-08-12 flag/migration reconciliation** (every flag verified ON against production by `npm run verify:flags`): Quest Engine/Rise Mode, pop-up engine, Executive Producer Sessions, live tips, Royalty Readiness. This line previously listed all five as dark, from code defaults rather than production.
