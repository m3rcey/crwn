...
- **`/` (the marketing homepage)** — since 2026-08-01 the homepage runs the **Opportunity Calculator** funnel, by mounting the SAME production component as `/tools/opportunity-calculator` (`PublicToolClient` with the registry config) rather than owning a second calculator. `src/app/page.tsx` → `src/app/HomeFunnel.tsx` = `HomeNav` + `<PublicToolClient config={opportunity-calculator} surface="homepage" below={<WorthExperience homepage marketingOnly />} />`. Section order: **photo hero → one primary CTA (smooth-scrolls to the wizard on the same page) → wizard → result → transition → builder → save/signup boundary → tool + CRWN showcase → every existing homepage marketing section** (unchanged, via `marketingOnly`, which strips that component's own nav/hero/Streaming Loss calculator and fires no analytics, so nothing double-counts). Homepage differences are chrome and attribution only: no "All tools" back control, `?ref=homepage` signup refs, and `surface: 'homepage'` on the shared funnel events (allowlisted in `sanitizeOpportunityMeta`) so homepage and tool traffic stay separable without renaming an event. The Streaming Loss Calculator still lives at `/worth`, unchanged. Authed visitors are redirected to `/home` by the middleware.
