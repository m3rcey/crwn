...
- **Storage:** track masters live in the PRIVATE Supabase Storage `audio` bucket (signed URLs, one hour; `src/lib/storage/signedAudio.ts` is the signer, `tracks_public` is the gate). Pages that read `tracks_public` server-side as the caller pre-sign in one batch (`attachStreamUrls`) so a tap plays with no round trip first; every other surface mints through `/api/tracks/[id]/stream`. R2 holds live VOD/thumbnails only. Uploads land raw in both `audio_url_*` columns; `scripts/transcode-audio.mjs` (daily GitHub Action `transcode-audio`, or by hand) gives each lossless stream file a 128 kbps MP3 copy and repoints only `audio_url_128`, so `audio_url_320` is the untouched master. Batch applied 2026-09-08.
