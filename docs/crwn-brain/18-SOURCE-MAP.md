...
- First paid conversion, all six rails: `src/lib/analytics/paidConversion.ts` (+ test), called from `src/lib/webhookHandlers.ts` (subscription, product, track, booking, live ticket, live tip). Stamps the artist's calculator so the event joins back to the funnel
