...
- **Founding-artist override:** `is_founding_artist` + unexpired `founding_fee_expires_at` → flat **5%** regardless of tier (`getArtistFeePercent()`, `platformTier.ts:245`). `Confirmed`. ⚠️ **The Founding Artist PROGRAM is retired (2026-07-14)** and all user-facing mentions are gone; zero artists ever carried the flag. The code path survives only because the **partner-code promo** (5% for 3 months, `platform-checkout:132`) deliberately reuses it. Keep/kill call pending in `TODO.md`; do not delete the path without it.
