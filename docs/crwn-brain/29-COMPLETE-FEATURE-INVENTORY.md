...
- **Why hybrid:** production proved twice (Ryan Leslie, Brent Faiyaz) that global keyword discovery structurally surfaces tiny superfan pages; artist search is a supplemental source, never the way to build the page universe. Index population, in priority order: **Discover Big Pages** (topic profile search + depth-1 related-profile expansion, returning a review table ranked by deterministic Seed Value; founder selects, Add Selected reuses the manual-add flow), automatic promotion at 50K+ followers from any search, manual handle adds, and reference-artist bootstrap (supplemental).
- **Boundary:** research and discovery only. No outreach automation, no private data, no LLM anywhere in the pipeline, no cron (all maintenance is founder-triggered), no auto-ingestion of discovery candidates.
- **Persistence:** all three tables (`distribution_pages`, `distribution_mentions`, `distribution_page_posts`) applied and probe-verified 2026-08-24. Discovery added no schema: candidates exist only in the review response until the founder adds them.
