...
Song Lab (GB experiment, `schema-phase2-song-lab.sql`, PENDING until the founder runs it): `song_lab_projects`, `song_lab_decisions` (options jsonb with stable a/b/c ids, `is_free` + `allowed_tier_ids`, open/close windows, artist-set `winning_option_id`), `song_lab_votes` (`UNIQUE(decision_id, fan_id)`, service-role writes only), `song_lab_offers` (per-artist free lead magnets, `UNIQUE(artist_id, slug)`), `song_lab_offer_claims` (`UNIQUE(offer_id, fan_id)`, `join_result` + `fresh_signup` attribution). Plus server-only `artist_profiles.song_lab_enabled` (no client grant, launch_partner pattern). Projects/decisions/offers are anon-SELECTable by policy (the public marketing surface); votes and claims are not.
