...
`campaigns`, `campaign_sends` (gains nullable `fan_id` + `contact_id` + recipient CHECK via UNRUN `schema-phase2-fan-invites.sql`, enabling invites to imported contacts), `fan_communication_prefs`; `sequences`, `sequence_steps`, `sequence_enrollments`, `sequence_sends`, `sequence_conversions`, `unsubscribe_events`; `platform_sequences`, `platform_sequence_steps`, `platform_sequence_enrollments`, `artist_notes` (platform CRM on artists); `crm_lists`, `crm_contacts`, `crm_outreaches`, `crm_outreach_sends`, `crm_outreach_unsubscribes`; `fan_contacts` (gains `consent_attested_at`/`consent_attestation_version` via the same unrun migration; contacts without attestation are never invitable); `fan_growth_notes`, `fan_growth_actions`; `saved_segments`; `smart_links`, `smart_link_captures` (+ pre-save `link_type`); `referral_clicks` (funnel); `email_suppressions`; `artist_marketing_costs`. `Confirmed`.
