...
### Distribution research (admin-only, 2026-08-24, migration PENDING)
`distribution_pages` (one row per observed public Instagram account: username unique, ig_user_id partial-unique, followers/verified/category/bio, `last_observed_at` for the 24h enrichment cache) + `distribution_mentions` (one row per artist-post observation: `artist_key` normalized artist name, `post_key` strongest stable post id, UNIQUE `(artist_key, post_key)` for dedupe, engagement columns NULL when Instagram hides them, `match_reason` for auditability). Feed the founder's Artist Distribution Finder on `/admin`; the compounding artist-to-page graph lives here. RLS admin-only SELECT, no write policies (service-role via `requireAdmin` routes only), ALL revoked from anon. `supabase/schema-phase3-distribution-finder.sql` — **not yet applied**; until then the finder searches live but caches nothing.
