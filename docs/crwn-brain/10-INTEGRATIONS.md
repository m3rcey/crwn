...
- **🔴 OPEN, found 2026-07-30: the configured credentials are Twilio's TEST pair, so no real SMS has ever sent.** Proven against the API: any real from-number returns error **20008 "Resource not accessible with Test Account Credentials"**, while a send from Twilio's magic test number (+15005550006) is accepted. The live Account SID is a *different value* from the test one (currently `AC285d…`), so BOTH `TWILIO_ACCOUNT_SID` and `TWILIO_AUTH_TOKEN` must be replaced with the live pair from the console dashboard's Account Info panel. Blast radius is small: `sms_subscribers` is empty, so no artist-facing SMS is affected; the only consumer is the hot-lead founder alert, which falls back to email. `sendSms` now detects code 20008 and returns that diagnosis instead of Twilio's misleading "not a valid message-capable number" text, which blames the from-number for a credentials problem.
