...
- **`25-POST-WIN-REFERRAL.md`** (2026-08-11): **ARCHITECTURE ONLY, NOT SHIPPED, no code exists.** The design for asking an artist to refer another artist immediately after a verified win, plus the evidence for why it was not built. The win itself is canonical (`first_paid_conversion`, all six paid rails, deduped per artist, already attribution-carrying). The blocker is attribution: there is no `artist_referrer` dimension, `ref` already belongs to the recruiter/partner rail whose `artist_referrals` table carries a **$50 flat fee**, and the documented durable carrier (`lead_magnet_results.input_data._attribution`) holds a value on **0 of 41** production rows. Two founder decisions gate it: whether an artist is ever paid for referring, and whether artist referrals ever enter the recruiter rail. Trigger population today is **2 artists** and the win has been recorded **0** times. Do not cite as live.
