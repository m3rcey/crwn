...
- **`29-COMPLETE-FEATURE-INVENTORY.md`** (2026-08-13): every feature CRWN ships, listed once, with why it exists, the benefit claimed, how it works, and **live production usage counts** read from the database that day. Written to be handed to an outside reviewer who decides what to cut for now, so it also carries the decision brief (four reversibility levels, the do-not-cut list, the duplication clusters). Useful on its own as the one place the product's scale (9 artists, 7 paying fans, 0 one-time purchases) sits beside its surface area (119 pages, 265 API routes, 26 crons, ~208k lines). Facts here are dated: re-read the counts before acting on them.
...
- **`25-POST-WIN-REFERRAL.md`** (updated 2026-08-12): **V1 LIVE, shipped 2026-08-12** (an earlier version of this line said "not shipped"; that was the stale claim, not the doc). Asks an artist to refer another artist immediately after a verified win (`first_paid_conversion`, all six paid rails). Ratified founder policy, pinned by `postWinReferral.test.ts` and MONEY-005 in the architecture registry: organic Post-Win referrals are **UNPAID FOREVER**, never retroactively commissionable, and the recruiter rail (`ref` / `artist_referrals` / `recruiter_payouts` / the $50 flat fee) is never touched. Attribution is the additive `artistReferrer` dimension carried as `?artist_ref=<slug>` (never `ref`), riding the existing `_attribution` carrier. Delivery is the `artist_post_win_referral` pop-up (priority 30, everyN 30d, max 2), correctly silent until a first paid conversion exists.
- **`26-PRODUCT-DRIFT-PREVENTION.md`** (2026-08-12): **LIVE.** The permanent drift-prevention system: the invariant registry (`src/lib/architecture/invariants.ts`, ~50 contracts with severity P0-P3), the centralized exception mechanism, the frozen compatibility identifiers, the feature reachability registry, the static migration-state contract, and `npm run verify:architecture` (the deterministic drift suite, ~2.5s, no credentials). Read its "Agent contract summary" before shipping anything that touches money, priority ownership, notifications, attribution, navigation, or identifiers. Changing a ratified rule follows the workflow in that doc: Brain rule first, then implementation, then registry, then tests — never "the test failed, so weaken the test".
...
- [ ] `npm run verify:architecture` passes (the drift suite; ~2.5s, doc 26). A failure is either real drift (fix the code) or an intentional rule change (follow doc 26's workflow) — never a reason to weaken the test.
