...
## Product maturity
**Live in production.** Core money loop is real and hardened. Codebase is **large and layered** (241 API routes, 25 crons, 134 migrations, 115 pages) and has grown faster than it's been consolidated — real dead/duplicate code and design-token drift exist. Current frontier (2026-08-03): the **evidence layer + Constraint Engine** — CRWN's first artist-facing closed feedback loop, which measures the journey through first paid conversion and per-tier fan behavior, then names the ONE thing blocking an artist and returns ONE action (pure, deterministic, no AI, never writes). Before it: the **Opportunity Funnel** (public tools → value-before-signup → journey resolver → experiments, live). The Quest Engine / Rise Mode is still dark. **`npm test` runs 820 vitest tests across 50 files (a moving figure: run it)**, all in the pure business layers; there is still no component/integration/e2e test, so `npm run build` remains the gate for everything the suite does not reach. → `13-CURRENT-STATE.md`
...
- **`23-ZERO-TO-ONE-STRATEGY.md`** (2026-08-10): the canonical strategic reference. The contrarian truth (an artist's income comes from a few hundred identifiable people, not a few hundred thousand followers, and is won or lost in how consistently those few hundred are operated over a year), the Fan Economy definition, the category (**Fan Economy Operating System**), the monopoly wedge (one sub-avatar, not four), the CRWN operating loop (Diagnose, Direct, Deliver, Measure, Learn), the intelligence flywheel and its one missing box, a skeptical moat assessment, and the Zero To One scorecard (current 3.9/10, potential 7.8/10). Homepage, calculator, onboarding and Manager copy should be derived from this, not written independently. `Not implemented.`
