...
| Opportunity Funnel layer | `src/lib/opportunityFunnels/{registry,analytics}.ts` — typed lifecycle/promotion **view** over the 17 tools (no re-registration); 7 `opportunity_*` + 16 journey + 9 personalized events on the existing sink; Own Your Fans = **primary** | **Production-ready (live)** |
