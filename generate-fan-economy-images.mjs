...
const STYLE_INSTRUCTION = "Use the exact same visual style as these reference images: bold black sharpie marker handwriting on pure white paper, clean hand-drawn icons and diagrams, high contrast black on white, no gray tones, no background texture. Match the lettering weight, spacing, and hand-drawn aesthetic exactly. CRITICAL MONOCHROME RULE: the entire image is BLACK INK ON WHITE PAPER and contains NO COLOUR WHATSOEVER. There is no gold, no yellow, no red, no blue, no green and no coloured accent anywhere, not on a chain, a logo, a garment, a record label, a highlight or any object. The attached person reference photographs ARE in colour and their colours must NOT be copied: translate every one of them into black marker line work. If a person wears a gold chain or coloured clothing in their photo, draw it in black ink like everything else. Every pixel in the finished page is either black ink or white paper. CRITICAL BACKGROUND RULE: The background must be PURE WHITE (#FFFFFF), absolutely flat, edge to edge. NO off-white, cream, eggshell, beige, or warm paper tones. NO desk, table, notebook, binding, or surface visible underneath. NO shadows under the page, NO page curl, NO page edges, NO paper texture or grain. The entire frame IS the paper — there is no visible surface, no background object, no hint that the paper is sitting on anything. This is a flat editorial scan, not a photograph of a sheet on a desk. Pure #FFFFFF pixels fill every edge of the frame. CRITICAL DELIMITER RULE: the quotation marks around a quoted line are DELIMITERS in this prompt, not ink on the page. Letter the words between them and never draw the quotation marks themselves, unless a mark appears INSIDE the quoted text, as in the single quotes around a keyword. A boxed note given as THE DIARY VAULT inside delimiters is lettered THE DIARY VAULT on the page, with no marks around it. CRITICAL WRAP RULE: when a quoted line is too long for one row and wraps onto the next row, the last word of the first row must NOT be repeated as the first word of the second row. Seen for real on this series: A GRAMMY OFF SOMEBODY ELSE'S ALBUM rendered as A GRAMMY OFF SOMEBODY SOMEBODY ELSE'S ALBUM, and HE RUNS HIS OWN COMPANY rendered as HE RUNS HIS OWN OWN COMPANY. Read the whole line once more after lettering it and delete any word that appears twice in a row. CRITICAL LONG-LINE TRANSCRIPTION RULE: the longest quoted lines are where words get dropped, doubled or mangled, and the boxed COMMENT line is the one line on the page that must be perfect, because it is how a viewer is told what to type. Transcribe every quoted line letter by letter, wrapping onto as many lines inside its box as it needs. Never drop a word, never repeat a word, never substitute a shorter word you half-read, and never carry a word down from the headline into another line. Seen for real on this series: COLLECTING rendered as NOTS, SHOWING UP rendered as SHOWING UP UP, and a headline's last word TELL welded onto the end of the comment line. CRITICAL BLACK-BELONGS-TO-OBJECTS RULE: the heavy black on the page is always SOMETHING. It fills clothing, hair, silhouette figures, shutters, safes, vaults, doorways and cast shadow inside a scene. Never lay down a large empty black band, slab, panel or filled rectangle that contains nothing and depicts nothing, and never black out a region of the page just to add contrast. If an area has no object in it, it stays white paper. CRITICAL CLOSED-VOCABULARY RULE: the ONLY words anywhere on the finished page are the exact words this prompt puts inside quotation marks. Every object in the scene is blank, INCLUDING objects you add that the prompt never described: a crate, box, banner, sign, poster, shelf label, garment, cap, bandana, durag, badge, screen or page you invent is drawn with NOTHING written on it. Never letter a word onto an object to explain what it is, never write a word you read on an attached reference photograph onto the clothing or headwear you draw from it, and never fill a sheet of paper or a notebook with imitation handwriting that forms readable words: draw plain wavy marker squiggle instead. Seen for real on this series: crates lettered DEAL, a durag lettered with its brand, a bandana lettered with a slogan, and a lyric sheet covered in invented nonsense words. CRITICAL HAIR RULE: hair that is DARK in the reference photo is filled SOLID BLACK with the same hand-coloured marker texture as the clothing, never left as unfilled white outlines. An outlined, unfilled head of hair reads as blond or grey on a black-and-white page and destroys the likeness of a person with dark hair, locs, braids or twists. Fill it. CRITICAL FONT RULE: ALL text in the image must be hand-drawn sharpie marker handwriting. NEVER use any printed, typeset, computer, Arial, Helvetica, serif, or sans-serif font anywhere in the image. Every single letter, number, word, label, header, list item, footer, and bottom tagline must look hand-written with a sharpie. No typography, no mixed fonts, no computer-generated text anywhere, not even in bottom taglines, captions, or footers. CRITICAL HEADLINE RULE: the single most common failure is the LARGEST headline text at the top of the page rendering as a clean printed, bold, or display font. The big hook headline MUST be thick, slightly uneven, hand-drawn sharpie capital lettering, exactly as if a person wrote it fast with a marker, with imperfect baselines and varying stroke widths. NEVER render the headline (or any text at any size) as a typeset, bold, or display font. Every size of text, especially the biggest headline, is hand-lettered by marker. MARKER FILL TEXTURE: any solid black or filled-in area (filled shapes, redaction bars, blacked-out regions, shaded pie slices, thick fills) must look HAND-FILLED with a real sharpie marker, NOT a flat digital fill. Show visible directional marker strokes, slightly uneven coverage, faint lighter streaks where the marker lifted, tiny specks and flecks of white paper showing through the fill, and slightly ragged stroke edges, exactly like a person colored the area in by hand with a marker that was running a little dry. NEVER render a solid area as a perfectly uniform, smooth, vector-flat black. This streaky texture lives INSIDE the black fills only; it is NOT gray shading and it does NOT change the page background, which stays pure flat white (#FFFFFF), and thin line work stays clean and bold.";
