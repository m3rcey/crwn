...
**Chained gaps + two payoff modes.** A gap does not have to be one question held to the very end. You can CHAIN: open Q1, close it, and *immediately* pose Q2 (an actual question sentence) so a loop is always open — the biggest beat still lands last. Closing a gap mid-script only fails if nothing re-hooks right after. And the payoff has TWO valid modes: (a) **shock-fact reveal** for artist-story reels — hold one surprising verified fact/number to the final line; (b) **how-to / method** for capability/BOFU scripts ("your fans spend $150", "here's the move") — pose a forward "so how do you ___?" question, then ANSWER it with the concrete moves broken down line by line (paid sub, sell merch direct, numbered vinyl, paid livestream + tips), which demos the CRWN capability without naming it. The viewer earns the method by asking for it first. Never close a gap and coast on a flat lesson/restate. (See memory `feedback_chained_gap_and_howto_payoff`.)
