...
**Lead with the QUESTION, not a setup statement.** Sentence 1 should BE the question (or a loaded line that forces it), not a statement followed by the question in sentence 2. "James Blake built a song on Kanye's album, then pulled his name off it. Why?" is weaker than opening on the question itself: "Why would a producer build a song on Kanye's new album, then pull his own name off it?" Put the question first; fold the setup details into it or reveal them right after. Where a template above is statement-first, prefer its question-led version.

**Open EVERY gap in ONE sentence — the opening hook AND every mid-script re-hook.** Fold the time-jump and the context INTO the question sentence; never spend a sentence (or two) setting up before you ask. BAD (3 sentences): "Now jump 19 years. Bully drops in 2026, and Kanye put it out a different way. So how much does he keep now?" GOOD (one sentence): "So nineteen years later, how much of that same dollar does Kanye keep on Bully, the album he put out a completely different way?"

**After the hook question, add a short tension line that promises the payoff.** Right after the opening gap (the hook question), drop a brief standalone beat of direct address that signals the answer is coming and pulls them in: "You bout to find out." / "Stay with me." / "Watch." (how-to mode often uses "Let's take a look."). It's its own short line, before the build. This is NOT a coast — it sits right after a freshly-OPENED gap that's still wide open, so it raises tension without resolving anything. Rotate the phrasing across scripts; don't reuse the same one every time.
...
**Make the surprise double as a capability demo (no CRWN mention).** Wherever you can, pick the surprising fact that ALSO implicitly models a real CRWN move WITHOUT naming CRWN and WITHOUT stating a lesson: a direct fan list you own (email/SMS capture), a premium one-off or numbered drop (tiers/scarcity), selling early/first access, a paid experience or appearance, products sold straight to the fanbase, fans recruiting fans (share-and-earn), going live for paying fans. The story shows the mechanism; the viewer feels the move without being told it. Models: Ryan Leslie's *$2M off 15,000 fans' phone numbers* = own the direct line; Nipsey's *1,000 copies at $100, sold out in a day, Jay-Z bought 100* = premium tier + scarcity; Berner's *$300M off the fanbase he rapped to* = products to your fans. Best-effort: a pure shock fact with no clean tie is still fine, but prefer the payoff that surprises AND demonstrates the capability. (See memory `feedback_crwn_capability_reverse_engineer`.)

**Chained gaps + two payoff modes.** A gap does not have to be one question held to the very end.

- **Chain the gaps.** Open Q1, close it, and *immediately* pose Q2 (an actual question sentence) so a loop is always open. **After ANY gap closes there are exactly two valid moves: re-hook with the next question, or END. Never a third thing.** Do not insert a restate, a confirming quote, an explanation of the irony, or a re-narration of what just happened — the viewer already got it, and a beat with no open loop is where they drop. **Deliver the answer EXACTLY ONCE. A confirming quote or paraphrase that re-says it is still a coast even if you glue it to the same sentence — CUT it, do not "fold" it in.** (e.g. once "the version wasn't his, AI over his work, Ye swore no AI" lands, Blake's quote "this aint what I created, I won't credit music I can't control" says the same thing again — delete it and re-hook.) This skill's statement-shaped template (HOOK→FORESHADOW→RISING→TWIST→PAYOFF) invites the coast in two spots: the TWIST beat becomes a restate instead of the re-hook question, and the PAYOFF beat gets a trailing "lesson/irony" sentence after the reveal. When chaining, the TWIST beat IS the next question; and END on the payoff line — no sentence after the reveal. The biggest beat lands last.
- **Mode A — shock-fact reveal** (artist-story reels): hold one surprising verified fact/number to the final line.
- **Mode B — how-to / method** (capability/BOFU scripts like "your fans spend $150", "here's the move"): pose a forward "so how do you ___?" question, then ANSWER it with the concrete moves. The method demos the CRWN capability without naming it; the viewer earns it by asking first. **Shape (from #24):** problem → "so how?" + a short transition ("Let's take a look") → the lanes broken down line by line, ordered for impact (lead with a big tangible one like merch; END the list on the full-circle conversion that flips the setup's weak item — e.g. the $11 streaming becoming a $15 paid sub) → then WITHHOLD the sum even though every component is shown: state the stakes ("the total lands on you, not Ticketmaster/the venue/Spotify"), RE-ASK it as a question ("what's the total when you add it up?"), and reveal the NUMBER last ("$150") as the held payoff. The viewer watched to see if the lanes actually stack to the whole budget, so make them WAIT for the number rather than stating the sum mid-script. In HOW-TO mode only, a single tight coda may follow the number ("you sold them the same things they already buyin, straight from you") — fine because it lands AFTER the payoff, and only if it's a fresh crystallization, not a re-narration. Shock-fact reels get NO coda: end on the reveal. The math must add up.

**Hold the answer for the END; never resolve a gap right after you open it.** The fatal move: pose the gap question ("so how do you go from pennies to the whole budget?") then IMMEDIATELY answer it in one line ("you sell em the same things they already buyin, straight from you"). That hands the payoff over up front, so there's no reason to watch on. The build/breakdown sits BETWEEN the question and the answer; the payoff the gap promised (the total landing on $150, the surprise) lands LAST. The breakdown can reveal the method piece by piece (that's the content), but the completion is held to the final beat. And make that final beat the SPECIFIC payoff (the number / total / surprise), not a generic one-line restate of the move the viewer could've predicted. (See memory `feedback_chained_gap_and_howto_payoff`.)
