...
**Never give the answer away — anywhere, including the last line.** Do NOT cap a how-to with a one-line thesis that summarizes the move (e.g. "you sell em the same things they already buyin, straight from you") — that line IS the answer, and once it's said (or if the viewer could predict it) there was no reason to wait. The test: could someone state your closer before watching the video? If yes, it's a give-away, not a payoff. The gap survives only while the specific answer (the number, the full tally, the surprise) is still AHEAD of them. Never close a gap and coast on a flat lesson, a restate, or a predictable thesis. (See memory `feedback_chained_gap_and_howto_payoff`.)
