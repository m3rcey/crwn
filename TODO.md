...
- [ ] **Watch the 23-second real-media prototype, then decide which Smino images CRWN may
      publish.** It is the test of the new direction ([docs/REEL_MEDIA_ARCHITECTURE.md](docs/REEL_MEDIA_ARCHITECTURE.md)):
      [videos/reels/23-smino-the-promise-you-forgot--proto/renders/](videos/reels/23-smino-the-promise-you-forgot--proto/renders/)
      (the highest `-vN.mp4` is the latest). It is INTERNAL ONLY: it carries the "Maybe In
      Nirvana" cover, marked uncleared, so its QA fails `publication_rights` on purpose and
      it was never exported. Your eye decides whether it reaches the reference class. Then
      the rights, which are what blocks a full reel: (1) Artist photos: the usable Creative
      Commons ones are few, soft and mostly 2016 (the only 2024 one is 618px wide); better,
      current photos need Smino's team or label to say yes (a press kit), because editorial
      licences such as Getty's generally exclude promotional use and a CRWN reel is
...
- **Fan Economy reel editor: after the founder's verdict on the prototype**, either rebuild
  ...
  first. Still owed either way: deleting the `figure`/`stage` code in `lib/world3d.js` (the
  storyboard no longer accepts a figure; the code itself is dormant, not yet removed).
