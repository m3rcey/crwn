## DRAKE LEFT UMG AND THE CATALOG STAYED BEHIND

**CAPTION:**

Drake Walked Out, The Catalog Stayed

Link in bio to own a fan list that walks out the door with you, not the label, with CRWN. Free to start at thecrwn.app.

---

**SCRIPT:**

What audience can Drake never take with him when he leaves a deal? He reportedly just walked out on Universal, and watch what stayed on the table. Reports say his catalog, the old albums, stay with the label. He walk out the door, the masters dont. Now this man got over 140 million followers on one app alone. Sound like the safest audience in the game, right? But he cant take none of em with him. Instagram own that list. TikTok own that list. The app change the rules tomorrow, that reach is gone. You dont own a follower. You rent em from a app that can die. Now flip it to you. Say you own 4,000 emails and numbers. 300 on a $12 tier, 40 on a $35 tier, a handful payin $90 for a call. That money moves with you to any app, any year. So your 4,000 is worth more than his 140 million, cause Drake got the biggest audience on earth and cant take a single one of em out the door.

---

**TITLES (suggested is in CAPTION above; alternates below):**
1. Drake Walked Out, The Catalog Stayed -- Structure 4  ← SUGGESTED
2. Drake Left A Catalog Behind -- Structure 4
3. Followers Vs A List You Own -- Structure 8
4. The Audience Drake Cant Take -- Structure 6
5. Every App Dies But A List -- Structure 5

**TOPIC:** Drake reportedly exited his UMG deal but his catalog stays with the label, and his 140M+ followers cant leave the platforms either; an owned email/phone list is the only audience asset that travels to any app or era.

---

**SCRIPT WITH VISUALS:**

**HOOK:** What audience can Drake never take with him when he leaves a deal? He reportedly just walked out on Universal, and watch what stayed on the table. [write "DRAKE" big at the top, draw a door to the right of it, write "UMG" on the inside of the door, draw an arrow showing a stick figure walking OUT through the door]

**FORESHADOW:** Reports say his catalog, the old albums, stay with the label. He walk out the door, the masters dont. [write "CATALOG" on the label's side of the door with a chain drawing around it, write "STAYS" under it and underline it]

**RISING ACTION:** Now this man got over 140 million followers on one app alone. Sound like the safest audience in the game, right? But he cant take none of em with him. Instagram own that list. TikTok own that list. The app change the rules tomorrow, that reach is gone. [write "140,000,000 FOLLOWERS" in the center, draw a phone outline around it, write "INSTAGRAM" and "TIKTOK" as two boxes, draw a big X over the phone, write "CANT EXPORT" next to the X]

**TWIST:** This what they dont tell you. You dont own a follower. You rent em from a app that can die. [cross out the word "OWN" and write "RENT" above it in bigger letters, draw a small tombstone with "APP" on it]

**PAYOFF:** Now flip it to you. Say you own 4,000 emails and numbers. 300 on a $12 tier, 40 on a $35 tier, a handful payin $90 for a call. That money moves with you to any app, any year. So your 4,000 is worth more than his 140 million, cause Drake got the biggest audience on earth and cant take a single one of em out the door. [write "YOUR LIST = 4,000" with an envelope and a phone drawn next to it, then a small stack: "300 x $12", "40 x $35", "calls $90", box the whole stack, draw an arrow from the box leading to three small app logos labeled "ANY APP" with a check mark]

---

**NANO BANANA PRO PROMPT:**

Flat scan of a white sheet of paper filling the entire frame. No desk, no surface, no edges visible, just white paper. Black sharpie marker handwriting. At the top, the word "DRAKE" written large in bold capitals, with a simple hand-drawn door to its right, the letters "UMG" written on the inside panel of the door, and a small stick figure with a smiley face walking out through the open door with an arrow showing the direction out. On the label's side of the door, the word "CATALOG" with a simple chain drawn looping around it, and beneath that the word "STAYS" underlined twice. In the center of the page, "140,000,000 FOLLOWERS" written bold, enclosed inside a simple hand-drawn rectangular phone outline, with two small boxes beside it labeled "INSTAGRAM" and "TIKTOK", a large X crossed over the phone, and the words "CANT EXPORT" written next to the X. Below that, the word "OWN" crossed out with the word "RENT" written larger above it, and a small hand-drawn tombstone with the word "APP" on it. Near the bottom, "YOUR LIST = 4,000" with a small drawn envelope and a small drawn phone beside it, then a short stacked list reading "300 x $12", "40 x $35", "calls $90", with a box drawn around the whole stack, and an arrow leading from the box to three tiny app squares labeled "ANY APP" with a check mark beside them. The background is pure white (#FFFFFF). The image is shot perfectly straight on, no angle, no shadow, no background elements. Pure white paper fills the entire 3:4 frame edge to edge.

---

**PAN ORDER:**

1. "DRAKE" big at the top with the door, "UMG" on the door, stick figure walking out
2. "CATALOG" with the chain around it, "STAYS" underlined
3. "140,000,000 FOLLOWERS" inside the phone outline, "INSTAGRAM" and "TIKTOK" boxes
4. The big X over the phone, "CANT EXPORT"
5. "OWN" crossed out, "RENT" above it, the "APP" tombstone
6. "YOUR LIST = 4,000" with envelope and phone
7. The stack "300 x $12", "40 x $35", "calls $90" boxed
8. Arrow to the three "ANY APP" squares with the check mark
