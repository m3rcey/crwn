## LUPE FIASCO FANS FREED LASERS FROM THE LABEL

**CAPTION:**

What Happens When You Cant Drop Your Own Album

Link in bio to release your music the day it's finished and sell it straight to your fans with CRWN. Free to start at thecrwn.app.

---

**SCRIPT:**

How did Lupe Fiasco's finished album get free from a label that refused to release it? Stay with me, cause the answer aint what you think. His third album Lasers was done. Reportedly sat finished for like two years. Atlantic wanted more pop, so they just held it. His own music, locked in a building he dont control. Then the fans did somethin wild. Almost 30,000 of em signed a petition. Then around 200 showed up outside Atlantic in New York. They called it Fiasco Friday. Lupe came out and told em the album finally got a date. When it dropped March 7th 2011, Lasers went straight to number one, his first ever. So that's how it got free. The fans marched in the street and made the label open the door it was holding shut.

---

**TITLES (suggested is in CAPTION above; alternates below):**
1. What Happens When You Cant Drop Your Own Album -- Structure 10 ← SUGGESTED
2. Lupe Fiasco Got Shelved By His Label -- Structure 2
3. Fans Freed A Finished Album -- Structure 4
4. Lupe Fiasco Made A Locked Album -- Structure 6
5. Petition Vs The Label -- Structure 8

**TOPIC:** Atlantic sat on Lupe Fiasco's finished album Lasers until fans protested it free; when a label can hold your music, it isn't really yours.

---

**SCRIPT WITH VISUALS:**

**HOOK:** How did Lupe Fiasco's finished album get free from a label that refused to release it? [draw a hand-drawn sharpie head-and-shoulders portrait capturing the recognizable likeness of Lupe Fiasco from the attached photo reference, drawn in confident sharpie line work, not a generic stick figure, labeled "LUPE FIASCO" underneath. top center, write "FINISHED ALBUM" big, then draw a simple padlock right under it with the album boxed and the lock on the box]

**FORESHADOW:** Stay with me, cause how it got loose aint what you think. [draw a small question mark to the right of the locked box]

**RISING ACTION:** His third album Lasers was done. Reportedly sat finished for like two years. [write "LASERS" inside the box, write "DONE" with a checkmark, then write "2 YRS" with a clock drawn next to it] Atlantic wanted more pop, so they just held it. His own music, locked in a building he dont control. [write "ATLANTIC" on the box as the label, draw a tall wall under the box] So the fans did somethin wild. Almost 30,000 of em signed a petition. [draw a few stick figures on the left, write "28,979 SIGNED" above them] Then around 200 showed up outside Atlantic in New York. They called it Fiasco Friday. [draw a stick figure holding a picket sign, write "FIASCO FRIDAY" on the sign]

**TWIST:** Lupe came out and told em the album finally got a date. March 7th, 2011. [write "MARCH 7, 2011" big, circle it] Now think about that. The fans had to march in the street to free a album that was already done. And when it dropped, Lasers went straight to number one. His first one ever. [draw a big "#1" on the right, write "FIRST EVER" under it, draw a crack splitting the wall]

**PAYOFF:** The music was ready the whole time. The label was the only wall between Lupe and the people who wanted it. [under the cracked wall, write "READY THE WHOLE TIME", draw an arrow from "LUPE" through the broken wall to a row of stick-figure fans on the far right]

---

**NANO BANANA PRO PROMPT:**

Flat scan of a white sheet of paper filling the entire frame. No desk, no surface, no edges visible, just white paper. Black sharpie marker handwriting. Near the top, a hand-drawn sharpie head-and-shoulders portrait capturing the recognizable likeness of Lupe Fiasco from the attached photo reference, drawn in confident sharpie line work, not a generic stick figure, labeled "LUPE FIASCO" underneath. At the top center the words "FINISHED ALBUM" written large in bold capital letters. Directly under it a hand-drawn square box with the word "LASERS" written inside it and the word "ATLANTIC" written along the top edge of the box, and a simple padlock drawn on the front of the box. Just under the box the words "DONE" with a checkmark next to it, and "2 YRS" with a small clock drawn beside it. Below the box the name "LUPE FIASCO" in bold capitals with a small question mark to its right. A tall brick wall drawn under the box with a jagged crack splitting down the middle of it, and the words "READY THE WHOLE TIME" written under the wall. On the left side three simple stick figures with the words "28,979 SIGNED" written above them. One stick figure holding up a rectangular picket sign with "FIASCO FRIDAY" written on the sign. In the middle the date "MARCH 7, 2011" written large and circled. On the right a large "#1" with "FIRST EVER" written underneath it. An arrow drawn from the "LUPE" name through the cracked wall pointing right to a row of three small stick-figure fans on the far right side. The background is pure white (#FFFFFF). The image is shot perfectly straight on, no angle, no shadow, no background elements. Pure white paper fills the entire 3:4 frame edge to edge.

---

**PAN ORDER:**

1. Sharpie portrait of Lupe Fiasco labeled "LUPE FIASCO"
2. "FINISHED ALBUM" title at top
3. Locked box / padlock under the title with question mark
4. "LASERS" inside the box with "DONE" checkmark and "2 YRS" clock
5. "ATLANTIC" label on the box and the wall under it
6. Three stick figures with "28,979 SIGNED"
7. Stick figure with "FIASCO FRIDAY" picket sign
8. "MARCH 7, 2011" circled in the middle
9. Cracked wall splitting open
10. "#1" with "FIRST EVER" on the right
11. "READY THE WHOLE TIME" and the arrow through the wall to the row of fans
