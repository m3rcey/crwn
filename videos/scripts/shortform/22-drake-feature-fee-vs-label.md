## DRAKE OWNS HIS MASTERS AND STILL RENTS HIS VOICE BY THE VERSE

**CAPTION:**

The Man Who Fought To Own His Masters Still Gets Paid Once On Your Song

Link in bio to own the songs you drop so every play pays you forever, not just once, with CRWN. Free to start at thecrwn.app.

---

**SCRIPT:**

Drake will hop on your song for one verse and take a single check to do it. And that one check is the worst seat in the room. The biggest name on the song, the smallest seat at the table.

A Drake verse is a one-time fee. You pay it, he sends it, the check clears, done. But the song he just blessed plays forever, and that forever money goes to whoever owns the song, not the guest who rapped on it.

Now here go the irony. Drake owns every one of his own masters since 2021. On his records he keeps the money that keeps coming. But the second he's a guest on your song, he's right back to taking the one check and walking.

The verse made the song hot. Owning it is what makes it pay. A fee gets paid once. A master gets paid every time somebody presses play.

And here go the kicker. Drake reportedly charge around a million dollars for one verse, the highest feature price in rap. The richest one-time check in the game, and it still lose to the artist who paid it. Cause that million stop the day it clear, and the song keep paying the owner for the next ten years.

---

**TITLES (suggested is in CAPTION above; alternates below):**
1. The Man Who Fought To Own His Masters Still Gets Paid Once On Your Song -- Structure 2  ← SUGGESTED
2. Drake's Verse Gets Paid Once, The Owner Gets Paid Forever -- Structure 3
3. Even Drake Rents His Voice By The Verse -- Structure 4
4. A Feature Fee Vs Owning The Song -- Structure 9
5. Why The Biggest Check In The Room Is The Worst One -- Structure 1

**TOPIC:** A Drake feature is a one-time fee, while the master royalties on that song pay whoever owns it for as long as it streams (~$4 per 1,000 plays). The irony: Drake has owned his own masters since 2021, so he collects forever money on his records, but as a guest on someone else's song he takes the one-time check. A fee is paid once; ownership pays every play.

---

**SCRIPT WITH VISUALS:**

**HOOK:** Drake will hop on your song for one verse and take a single check to do it. And that one check is the worst seat in the room. The biggest name on the song, the smallest seat at the table. [write "ONE DRAKE VERSE" at the top, then "> A DECADE OF CHECKS" underneath and underline it]

**FORESHADOW:** The biggest name on the song, the smallest seat at the table. [write "THE BIG CHECK = THE WORST SEAT?" in caps with a question mark]

**RISING ACTION:** A Drake verse is a one-time fee. You pay it, he sends it, the check clears, done. [draw a stick figure labeled "DRAKE" walking out of a door labeled "STUDIO" holding one check labeled "PAID ONCE"] But the song he blessed plays forever, and that forever money goes to whoever owns the song, not the guest who rapped on it. [draw a money bag labeled "OWNER" with arrows of plays flowing into it, and a separate small "GUEST" figure with a single check and an X after it]

**TWIST:** Now here go the irony. Drake owns every one of his own masters since 2021. On his records he keeps the money that keeps coming. [write "HIS RECORDS: DRAKE = FOREVER MONEY" boxed] But the second he's a guest on your song, he's right back to taking the one check and walking. [write "YOUR RECORD: DRAKE = ONE CHECK" boxed and circle it]

**MID PAYOFF:** The verse made the song hot. Owning it is what makes it pay. A fee gets paid once. A master gets paid every time somebody presses play. [write "FEE = PAID ONCE" on the left, "MASTER = PAID EVERY PLAY" on the right, with a vertical line between] [write "VERSE = MAKES IT HOT" and "OWNING IT = MAKES IT PAY" stacked inside one rough oval]

**PAYOFF:** And here go the kicker. Drake reportedly charge around a million dollars for one verse, the highest feature price in rap. The richest one-time check in the game, and it still lose to the artist who paid it. Cause that million stop the day it clear, and the song keep paying the owner for the next ten years. [write "DRAKE VERSE ≈ $1,000,000" then "HIGHEST FEE IN RAP" boxed, then "STILL LOSES TO THE OWNER" in big caps at the bottom and circle it]

---

**NANO BANANA PRO PROMPT:**

Flat scan of a white sheet of paper filling the entire frame. No desk, no surface, no edges visible, just white paper. Black sharpie marker handwriting. At the top of the page, "ONE DRAKE VERSE" is written in bold capital letters. Below that, "> A DECADE OF CHECKS" is written in capitals and underlined. Below that, "THE BIG CHECK = THE WORST SEAT?" is written in capitals with a question mark. In the upper middle of the page, a stick figure labeled "DRAKE" is drawn walking out of a hand-drawn door labeled "STUDIO" holding one check labeled "PAID ONCE". Below that, "~$4 PER 1,000 PLAYS" is written in capitals. Below that, a stacked list is written: "1,000,000 PLAYS = $4,000", "100,000,000 PLAYS = $400,000". Render every digit exactly as written, do not change any numbers. Below that, a hand-drawn money bag labeled "OWNER" is drawn with several hand-drawn arrows labeled "PLAYS" flowing into it, and to the side a small separate stick figure labeled "GUEST" is drawn holding a single check with a sharpie X after it. In the middle of the page, "HIS RECORDS: DRAKE = FOREVER MONEY" is written in capitals and boxed. Below that, "YOUR RECORD: DRAKE = ONE CHECK" is written in capitals, boxed, and circled. Near the bottom, a vertical line splits the page: on the left "FEE = PAID ONCE" is written, on the right "MASTER = PAID EVERY PLAY" is written. At the very bottom of the page, "VERSE = MAKES IT HOT" is written and below it "OWNING IT = MAKES IT PAY" is written, both circled together inside one rough sharpie oval. The background is pure white (#FFFFFF). The image is shot perfectly straight on, no angle, no shadow, no background elements. Pure white paper fills the entire 3:4 frame edge to edge.

---

**PAN ORDER:**

1. "ONE DRAKE VERSE" / "> A DECADE OF CHECKS" underlined
2. "THE BIG CHECK = THE WORST SEAT?"
3. Drake walking out STUDIO with "PAID ONCE" check
4. Money bag "OWNER" with plays flowing in / "GUEST" with one check and X
5. "HIS RECORDS: DRAKE = FOREVER MONEY" boxed
6. "YOUR RECORD: DRAKE = ONE CHECK" boxed and circled
7. "FEE = PAID ONCE" / "MASTER = PAID EVERY PLAY"
8. "VERSE = MAKES IT HOT" / "OWNING IT = MAKES IT PAY" in oval
9. "~$4 PER 1,000 PLAYS"
10. "1,000,000 PLAYS = $4,000" / "100,000,000 PLAYS = $400,000" circled at bottom
