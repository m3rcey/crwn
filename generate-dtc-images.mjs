...
console.log(`Generating ${scriptFiles.length} DTC-audit images for scripts ${START_SCRIPT_NUMBER}-${END_SCRIPT_NUMBER}`);
...
for (let idx = 0; idx < scriptFiles.length; idx++) {
  ...
  // Single-artist audits: only bind the artist named in the filename, so
  // incidental alias matches (e.g. "Baby" -> Birdman) never attach the wrong face.
  const peopleSlugs = findMentionedSlugs(scriptContent).filter((s) => slug.includes(s));
